...
dropdowns.forEach((dropdown) => {
  dropdown.addEventListener("click", (e) => {
    if (e.target.tagName === "DIV")
      e.target.lastElementChild.classList.toggle("d-flex");
  });
});
...
function display(obj) {
  ...
  if (actualExp === "" && (type === "operator" ||type ==="special-operator")) {
    showAlert("Enter operand first");
    return;
  } else if(actualExp.slice(-1)==="(" && (type === "operator" ||type ==="special-operator")){
    return;
  }
  else {
    if (
      ...
      } else if (actualExp.slice(-1) === "-"  && calc.operators.includes(actualExp[actualExp.length-2])) {
        return;
      } else {
    ...
    switch (type) {
      ...
      case "factorial-func":
        ...
        let ans = fact(n);
        actualExp = actualExp.replace(replacePart, ans);
        break;
    }
...
function memoryHandler(obj) {
  ...
  let MCBtn = parent.firstElementChild.firstElementChild;
  let MRBtn = parent.firstElementChild.nextElementSibling.firstElementChild;
  let MSBtn = parent.lastElementChild.firstElementChild;
  let val = obj.getAttribute("value");
  switch (val) {
    case "MS":
      if(!input.value) input.value=0;
      else input.value = evaluate(actualExp);
    ...
    case "M+":
      input.value = evaluate(actualExp);
      if (isNaN(input.value)) {
        input.value = "0";
      }
      memoryDisplay.value = Number(memoryDisplay.value) + Number(input.value);
      memoryDisplay.parentElement.classList.remove("invisible");
      MCBtn.disabled = false;
      MRBtn.disabled = false;
...
function showAnswer() {
  ...
  } else if (Math.trunc(res).toString().length >= 15) {
...
function showAlert(msg) {
  alertDisplay.parentElement.classList.remove("invisible");
  alertDisplay.value = " ! " + msg;
  currentDisplay.value="";
  actualExp="";
  input.value=""
  setTimeout(() => {
    alertDisplay.parentElement.classList.add("invisible");
  }, 1500);
}
  ...
  if(input.value.slice(-1)===")"){
    ...
    if(input.value[start-2] ){
      if(input.value[start-1]!=="-"){
        ...
        let x=evaluate(getVal(actualExp));
        actualExp=actualExp.slice(0,actualExp.lastIndexOf("("))+x*-1;
      }else if(input.value[start-1]==="-" && calc.operators.includes(input.value[start-2])){
        input.value=input.value.slice(0,start-1)+input.value.slice(start);
        let x=evaluate(getVal(actualExp));
        if(x<0 && !calc.operators.includes(actualExp[actualExp.lastIndexOf("-")-1]))
...
function getVal(exp) {
  if (exp[exp.length - 1] === "!") {
    if(exp[exp.length - 2]===")"){
      return [exp.slice(exp.lastIndexOf("(")), evaluate(getVal(exp.slice(0,-1)))];
    }
    let i = exp.length - 1;
